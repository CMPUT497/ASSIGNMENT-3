...
Our qualitative analysis identified four critical failure modes in the LLM output. First, the model frequently defaulted to generic verbs like ``\foreignlanguage{urdu}{کرنا}'' (to do) instead of specific content words. Second, we observed significant input leakage, exemplified by outputs like ``you:coerce'' where English formatting contaminated the target text. Third, the model disregarded lemma constraints, producing multi-word phrases or complex agentive nouns such as ``\foreignlanguage{urdu}{جدوجہدکرنےوالا}'' (\textit{struggler}) rather than concise roots. Most critically, the model suffered from morphological fragmentation, repeatedly outputting ``\foreignlanguage{urdu}{دری}'' (dari) a non-lexical suffix or sub-word unit across disparate Sense IDs, failing to construct complete, valid Urdu lemmas.
...
However, our manual qualitative analysis of the 10-sentence sample revealed mixed results. From the 10 sentences, ExpandNet projected a total of 39 senses, out of which 27 were found to be correct. The remaining 12 projections were incorrect. Upon investigating these errors, we found that the majority of incorrect projections involved English content words specifically nouns, company names, and country names being wrongly aligned to Urdu filler words. For instance, the noun ``report'' was incorrectly aligned to the function word ``\foreignlanguage{urdu}{ایک}'' (one), and the adjective ``American'' was mapped to the preposition ``\foreignlanguage{urdu}{میں}'' (in). Additionally, we observed issues with multi-word concepts; for instance, terms like ``death penalty'' require multiple words in Urdu to convey the full meaning, yet the system often projected only a single word like ``\foreignlanguage{urdu}{موت}'' (death), rendering the translation incomplete or incorrect.
